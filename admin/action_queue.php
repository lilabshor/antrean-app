...
try {
    ...
    if ($action === "call_next") {
        $sql    = "SELECT id, queue_code FROM queues WHERE appointment_date = ? AND status = 'WAITING'";
        $params = [$today];
        ...
        if ($next_queue) {
            ...
            $_SESSION["flash_err"] = "Tidak ada antrean yang sedang menunggu.";
        }
        ...
        $_SESSION["flash_msg"] = "Panggilan ulang telah dikirim ke layar display.";
        ...
        $stmt->execute([$queue_id, $counter_number]);
        $_SESSION["flash_msg"] = "Pelayanan antrean berhasil diselesaikan.";
        ...
        $stmt->execute([$queue_id, $counter_number]);
        $_SESSION["flash_msg"] = "Antrean ditandai telah dilewati / tidak hadir.";
    }
    ...
    $_SESSION["flash_err"] = "Terjadi kesalahan saat memproses antrean.";
}
